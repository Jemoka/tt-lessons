...
- **Corrects a prior lesson** ([2026-06-03-ttxla-embedding-bw-tile-padding-grad](/home/houjun/lessons/2026-06-03-ttxla-embedding-bw-tile-padding-grad/README.md)):
  it characterized the same op's non-aligned behavior but tested **only the all-ones (`sum`) cotangent**, saw a small
...
- **Concrete blast radius:** mainline GPT configs use `vocab_size = 100288 = 32×3134` — tile-aligned
  (cl100k's 100277 deliberately padded to a 32-multiple) — so `gpt/train/pretrain` was **never** hit by
  this (it trains `wte` correctly on TT pre-fix, consistent with the "completes end-to-end" result).
  The affected configs are ones with non-padded vocabs (e.g. 1059 / 291 / 163 / 67 — all `%32 = 3`),
  which would inf on TT with an unfrozen `wte` before this fix.

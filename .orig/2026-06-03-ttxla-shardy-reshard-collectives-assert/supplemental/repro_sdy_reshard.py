"""Pure-JAX reproducer for the Shardy reshard_to_collectives `isDone()` assertion
that aborts the theseus GPT trainer on TT.

The theseus trainer annotates weight kernels for tensor parallelism on a
<"batch"=1,"shard"=1> mesh — e.g. c_attn kernel [{}, {"shard"}] feeding a matmul
whose other operand wants [{"shard"}, {}]. Even with shard=1 (one device) this
forces an sdy.ReshardOp; tt-xla lowers it through Shardy's reshard->collectives
pass, which asserts: reshard_to_collectives.cc:394 `isDone()`.
"""
import os
